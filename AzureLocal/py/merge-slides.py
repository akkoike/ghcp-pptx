...
表紙のタイトルを「Azure Local 詳細仕様」に変更して最終 .pptx を出力するスクリプト
"""
import os
import copy
...
CONTENT_PPTX = os.path.join(ROOT_DIR, 'docs', 'AzureLocal_content.pptx')
OUTPUT_PPTX = os.path.join(ROOT_DIR, 'docs', 'AzureLocal.pptx')

TITLE_TEXT = 'Azure Local 詳細仕様'


def copy_slide(src_prs, src_slide, dst_prs):
    """src_slide を dst_prs の末尾にコピーする"""
    # スライドレイアウトを取得（最初のレイアウトを使用）
